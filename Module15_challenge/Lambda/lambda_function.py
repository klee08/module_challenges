    ...
    if investment_amount is None:
        ...
        if investment_amount < 5000:
            return build_validation_result(
                False,
                "investmentAmount",
                "The investmentAmount should be greater than or equal to 5000(USD), "
                "please provide a correct investmentAmount.",
            )
    if risk_level is None:
        return build_validation_result(
                False,
                "riskLevel",
                "What level of investment risk would you like to take? (None, Low, Medium, High)?",
        )
    elif str(risk_level).upper().strip() not in ["NONE", "LOW", "MEDIUM", "HIGH"]:
        return build_validation_result(
                False,
                "riskLevel",
                "What level of investment risk would you like to take? (None, Low, Medium, High)?",
...
def get_recommendation(risk_level):
    ...
    #respond with an investment recommendation based on the selected risk level as follows:
    #* **none:** "100% bonds (AGG), 0% equities (SPY)"
    #* **low:** "60% bonds (AGG), 40% equities (SPY)"
    #* **medium:** "40% bonds (AGG), 60% equities (SPY)"
    #* **high:** "20% bonds (AGG), 80% equities (SPY)"
    # Allow both upper or lower case, or with whitespace 
    if risk_level  == 'NONE':
